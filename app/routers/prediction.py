...
@router.get("/news/{news_id}/impact/{user_id}", response_model=NewsModel)
...
@router.post("/process-all-news/{user_id}", response_model=Dict)
async def process_all_news_for_user(user_id: str, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(_process_all_news_background, portfolio)
...
@router.get("/portfolio/{user_id}/impact", response_model=Dict)
